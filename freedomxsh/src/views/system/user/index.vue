 ...
 * @LastEditTime: 2021-05-27 09:58:35
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \vue-rbac-admin\src\views\system\user\index.vue
-->
<template>
  <div>
    <search-form label-width="80px" :model="query" :form-cell="searchCell" :is-mobile="mobile" @on-search="handleSearchSubmit" />
    <operation title="用户列表" :radio-value="null" :btn-group="btnGroup" />
    <base-table
      ref="baseTable"
      :loading="loading"
      :data="data"
    ...
    >
      <pagination :total="total" :page.sync="query.page" :limit.sync="query.limit" :is-mobile="mobile" @pagination="getList" />
    </base-table>
    <form-dialog
      :detail="detail"
...
export default {
  ...
  mounted() {
    ...
    if (this.userInfo && this.userInfo.type === 0) {
      ...
      Promise.all([this.getList(), this.getAllCompany(this.userInfo.gasRegionId)]).then(() => {
        this.loading = false;
      });
    }
  },
  methods: {
    ...
    handleSearchSubmit() {
      this.query.page = 1;
      this.getList();
      console.log(this.query, this.searchCell, '213123124123');
    },
